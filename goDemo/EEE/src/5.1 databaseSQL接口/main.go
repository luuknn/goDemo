...
/*
对许多web应用程序而言 数据库是其核心所在 数据库几乎可以用来存储你想查询和修改的任何信息 比如用户信息 产品目录 或者新闻列表等
Go没有内置的驱动支持任何的数据库 但是 Go定义了database/sql接口
用户可以基于驱动接口 开发相应数据库的驱动
5.1 小节 里面介绍Go设计的一些驱动 介绍Go是如何设计数据库驱动接口的
5.2-5.4 小节 介绍目前使用比较多的一些关系型数据驱动以及如何使用
5.5 小节介绍我自己开发的一个ORM库 基于database/sql标准接口开发 可以兼容几乎所有支持database/sql的数据库驱动
可以方便的使用Go style来进行 数据库操作
目前nosql已经成为web开发的一个潮流 很多应用采用了nosql作为数据库 而不是以前的缓存 5.6小节将介绍mongodb和redis两种nosql数据库

Go与php不同的地方是Go没有官方提供的数据库驱动  而是为开发者开发数据库驱动定义了 一些标准接口
开发者可以根据等一的接口来开发相应的数据库驱动 这样做有一个好处 就是只要按照标准接口开发的代码
以后需要迁移数据库时  不需要进行任何修改那么go定义了哪些标准接口呢 让我们来详细的分析一下
 sql.Register
 这个存在于databas/sql 的函数是用来注册数据库驱动的 当第三方开发者开发数据库驱动时  都会init函数 在init里面
 会调用这个Register(name string,driver driver.Driver)完成本驱动的注册
 我们来看一下 mymysql sqlite3的驱动里面都是怎么调用的
 //http://github.com/mattn/go-sqlites驱动
 func init(){
