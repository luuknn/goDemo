...
//我们平常浏览网页的时候 会打开浏览器 输入网址后按下回车键 然后就会显示出你想要的内容
//对于普通的上网过程 系统其实是这样做的 浏览器 本身就是一个客户端 当你输入url的时候 浏览器会去请求DNS服务
//通过DNS获取相应的域名对应的IP  然后 通过ip地址 找到ip对应的服务器后 要求建立 TCP连接 等浏览器 发送完HTTP Request(请求包) 服务器接收到请求包 之后才开始
//处理请求包  服务器调用自身服务 返回HTTP Response 响应包 客户端收到来自服务器的响应之后 开始渲染这个Response包里的主体(body) 等收到全部的内容随后断开与该服务器之间的TCP连接

//WEB服务器的工作原理 可以简单地归纳为
//客户机通过tcpip协议建立到服务器的TCP连接
//客户端向服务器发送HTTP请求包 请求服务器里的资源文档
//服务器向客户端发送http协议应答包 如果请求的资源包含有动态语言的内容 那么服务器会调用动态语言的解释引擎负责处理 动态内容 并将处理得到的数据返回给客户端
//客户机 与服务器 断开 由客户端解释 HTML文档 在客户端屏幕上 渲染图形结果

//一个简单的HTTP事务就是这样实现的 看起来很复杂 原理其实挺简单的  需要注意的是 客户机与服务器之间的通信是非持久的 也就是当服务器发送了应答后就与客户机断开连接 等待下一次请求

//URL 和 DNS 解析
//我们浏览网页都是通过url访问的 那么url到底是什么样的呢
//URL uniform resource locator 统一资源定位符 的英文缩写 用于描述 一个网络的资源 基本格式如下
//scheme://host[:port]/path0/...[?query-string][#anchor]
//scheme 指定底层使用的协议 例如 http https ftp
//host HTTP服务器的ip地址 或者域名
//port # HTTP服务器的默认端口是80 这种情况下 端口号 可以省略 如果使用了别的端口 必须指明 例如 http://abc.com/8080/
//path 访问资源的路径
//query string 发送给http服务器的数据
//anchor 锚

//DNS domain name system 是域名系统的英文缩写 是一种组织成 域层次结构的计算机和网络服务命名系统
//它用于TCP/IP网络 它从事 将主机名 或域名 换为 实际的IP地址工作 DNS 就是这样的一位翻译官 它的基本工作原理可用下图来表示
